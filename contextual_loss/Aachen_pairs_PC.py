...
mode_folder = osp.join(root_folder,'perceptual_loss')
...
for idx,pair in enumerate(pairs):
    ...
    for num_styles in [0,4,8,16]:
        ...
        _ = model(input_day)
        day_feats = feats_go.feats.clone()[0].cpu().data.numpy()
        _ = model(input_night)
        night_feats = feats_go.feats.clone()[0].cpu().data.numpy()

        C,H,W = day_feats.shape

        day_feats = np.reshape(day_feats,(C,H*W))
        night_feats = np.reshape(night_feats,(C,H*W))

        day_feats_sq = np.sum(day_feats**2,axis=1)
        night_feats_sq = np.sum(night_feats**2,axis=1)
        product = np.sum(day_feats*night_feats,axis=1)

        perceptual_loss = np.sum((day_feats_sq - 2*product + night_feats_sq)/(C*H*W))
