...
for auto_layer in range(1 + (not args.automode)*(args.layer - 1), 5 + (not args.automode)*(args.layer - 5 + 1)):
    ...
    for auto_block in range((not args.automode)*(args.block),
        ...
        for input_img in imgs_torch:
            ...
            deconv_resnet = Deconv_ResNet(identity_maps)
            deconv_resnet.eval()
            ...
            for layer in range(1,4+1):
                ...
                for blk in range(0,num_blocks_conv[layer-1]):
                    ...
                    #print(conv_layer_str,deconv_layer_str)
                    #print(blk,deconv_blk_idx)
                    ...
                    #print(deconv_blk._modules['deconv1'].weight.data==deconv_layers['layer'+str(4-layer+1)][num_blocks_conv[layer-1]-blk-1]._modules['deconv1'].weight.data)
            ...
            actv_sele,visual_results,max_index = visualize(auto_layer,auto_block,activations,deconv_resnet,num_maps = 1)
        ...
        for row in range(n_plts):
            for col in range(n_plts):
                ...
                ax2[row,col].imshow(enhance(vis_results_all_imgs[row*3+col][0]))
                coordinates = vis_results_all_imgs[row*3+col][1]
