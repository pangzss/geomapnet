...
class SwitchWhiten2d(Module):
    ...
    def __init__(self,
        ...
        self.num_pergroup = num_features
        self.num_groups = self.num_features // self.num_pergroup
        ...
        self.sw_mean_weight = Parameter(torch.ones(num_features))
        setattr(self.sw_mean_weight, 'bin_gate', True)
        if not self.tie_weight:
            self.sw_var_weight = Parameter(torch.ones(num_features))
            setattr(self.sw_var_weight, 'bin_gate_var', True)
        else:
        ...
        self.register_buffer('running_mean',
                             torch.zeros(self.num_groups, self.num_pergroup, 1))
        self.register_buffer(
            'running_cov',
            torch.eye(self.num_pergroup).unsqueeze(0).repeat(self.num_groups, 1, 1))
    ...
    def forward(self, x):
        ...
        if self.sw_type == 2:
            ...
            #    self.eps * eye
            self.sw_mean_weight.data.clamp_(0.,1.)
            self.sw_var_weight.data.clamp_(0.,1.)

            bn_mean_w = self.sw_mean_weight[None,:,None].expand(mean_bn.size())
            in_mean_w = 1. - bn_mean_w

            bn_var_w = self.sw_var_weight[None,:,None].expand(cov_bn.size())
            in_var_w = 1. - bn_var_w

            mean = bn_mean_w * mean_bn + in_mean_w * mean_in
            cov = bn_var_w * cov_bn + in_var_w * cov_in + \
                self.eps * eye
