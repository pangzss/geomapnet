...
class Trainer(object):
    def __init__(self, model, optimizer, train_criterion, config_file, experiment,
        ...
        if self.config['log_visdom']:
            ...
            self.pose_loss_win = 'pose_loss_win'
            self.vis = Visdom(server=visdom_server, port=visdom_port)
            self.vis.line(X=np.zeros((1, 1)), Y=np.zeros((1, 1)), win=self.pose_loss_win,
                          opts={'legend': ['pose_loss'],
                                'xlabel': 'epochs',
                                'ylabel': 'loss'}, env=self.vis_env)

            self.triplet_loss_win = 'triplet_loss_win'
            self.vis.line(X=np.zeros((1, 1)), Y=np.zeros((1, 1)), win=self.triplet_loss_win,
                          opts={'legend': ['triplet_loss'],
                                'xlabel': 'epochs',
                                'ylabel': 'loss'}, env=self.vis_env)

            self.perceptual_loss_win = 'perceptual_loss_win'
            self.vis.line(X=np.zeros((1, 1)), Y=np.zeros((1, 1)), win=self.perceptual_loss_win,
                          opts={'legend': ['perceptual_loss'],
                                'xlabel': 'epochs',
                                'ylabel': 'loss'}, env=self.vis_env)

            self.content_loss_win = 'content_loss_win'
            self.vis.line(X=np.zeros((1, 1)), Y=np.zeros((1, 1)), win=self.content_loss_win,
                          opts={'legend': ['content_loss'],
                                'xlabel': 'epochs',
                                'ylabel': 'loss'}, env=self.vis_env)

            self.content_loss_win = 'style_loss_win'
            self.vis.line(X=np.zeros((1, 1)), Y=np.zeros((1, 1)), win=self.style_loss_win,
                          opts={'legend': ['style_loss'],
                                'xlabel': 'epochs',
                                'ylabel': 'loss'}, env=self.vis_env)
            ...
            self.val_loss_win = 'total_loss_win'
            self.vis.line(X=np.zeros((1, 1), Y=np.zeros((1, 1)), win=self.total_loss_win,
                          opts={'legend': ['total_pos'], 'xlabel': 'epochs',
                                'ylabel': 'loss'}, env=self.vis_env))
            ...
            if len(val_pos_list) >= 2 and ((epoch % self.config['val_freq'] == 0) or
                ...
                if self.config['val_freq'] == self.config['snapshot']:
                    ...
                    if (last_pos_change + last_ori_change) > 0:
                        ...
                        print(
                            f'Validation loss decreased ({last_saved_pos:.6f} --> {curr_pos:.6f}) ({last_saved_ori:.6f} --> {curr_ori:.6f}). Zero counter and save model ...')
                    ...
                    if batch_idx % self.config['print_freq'] == 0:
                        ...
                        print('Train {:s}: Epoch {:d}\t' \
                              ...
                              format(self.experiment, epoch, batch_idx, len(self.train_loader) - 1,
                                     ...
                                     loss.triplet_loss.item(), loss.perceptual_loss.item(), loss.content_loss.item(),loss.style_loss.item(),
                                     (loss.abs_loss[0] + loss.abs_loss[1] + loss.vo_loss).item(),
                                     loss.final_loss.item(), lr))

                        if self.config['log_visdom']:
                            self.vis.line(X=np.asarray([epoch_count]),
                                          Y=np.asarray([(loss.abs_loss[0] + loss.abs_loss[1] + loss.vo_loss).item()]), win=self.pose_loss_win,
                                          name='pose_loss',
                                          update='append', env=self.vis_env)
                            self.vis.line(X=np.asarray([epoch_count]),
                                          Y=np.asarray([loss.triplet_loss.item()]), win=self.triplet_loss_win,
                                          name='triplet_loss',
                                          update='append', env=self.vis_env)
                            self.vis.line(X=np.asarray([epoch_count]),
                                          Y=np.asarray([loss.perceptual_loss.item()]), win=self.perceptual_loss_win,
                                          name='perceptual_loss',
                                          update='append', env=self.vis_env)
                            self.vis.line(X=np.asarray([epoch_count]),
                                          Y=np.asarray([loss.content_loss.item()]), win=self.content_loss_win,
                                          name='content_loss',
                                          update='append', env=self.vis_env)
                            self.vis.line(X=np.asarray([epoch_count]),
                                          Y=np.asarray([loss.style_loss.item()]), win=self.style_loss_win,
                                          name='style_loss',
                                          update='append', env=self.vis_env)
                            self.vis.line(X=np.asarray([epoch_count]),
                                          Y=np.asarray([loss.total_loss.item()]), win=self.total_loss_win,
                                          name='total_loss',
                                          update='append', env=self.vis_env)
...
def step_feedfwd(data, model, cuda, target=None, criterion=None, optim=None,
    ...
    with torch.set_grad_enabled(train):
        ...
        pred = output[0]
        feats = output[1]
        #feats = feats if train else [None,0,0]
