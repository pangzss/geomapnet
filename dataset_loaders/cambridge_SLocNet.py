...
class CambridgeSLocNet(data_.Dataset):
    ...
    def __init__(self, data_path, train, overfit=None, scene='ShopFacade',
        ...
        print('real_prob: {}.\nstyle_dir: {}\nnum_styles: {}'.format(self.real_prob, self.style_dir,
                                                                     len(self.available_styles) \
        ...
        for p in self.points:
            pose = process_poses_quaternion(np.asarray([p.position]), np.asarray([p.rotation]), mean_t, std_t,
                                            np.eye(3), np.zeros(3), np.ones(1))
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            anchor_style = self.get_style()
            resize = transforms.Resize(anchor.size()[-2:])
            style = self.transform(resize(anchor_style))

            real_triplet = torch.stack((pos, anchor, neg, style), dim=0)
            pose_triplet = torch.stack((pos_pose, anchor_pose, neg_pose), dim=0)
            return (real_triplet, 0), pose_triplet
        else:
